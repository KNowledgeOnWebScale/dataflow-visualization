...
|  `position`  | `{x: _, y: _}`<br/>The values of x and y can by any number. |                                   `{x: 0, y: 0}`<br/> Except for when `autoLayout` is `true` in the global default settings of when `vgroup` or `hgroup` is used.                                   | The position of the node.                                                                                                                                                                                                              |
...
[As seen before](#nodes), you can set the positions of all nodes individually with the `location` key. But it is not
